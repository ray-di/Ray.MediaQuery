...
> **🏗️ アーキテクチャパターン**: Ray.MediaQueryは[**Business Domain Repository Pattern (BDRパターン)**](./docs/BDR_PATTERN-ja.md)を可能にします。これは、依存性注入とビジネスロジック統合を通じて、シンプルなデータベースクエリを豊富なドメインオブジェクトに変換するアプローチです。
